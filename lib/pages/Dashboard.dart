...
// initialise the plugin. app_icon needs to be a added as a drawable resource to the Android head project
              ...
              await Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) => Notifications(
                    groupId: data.documents[0]['groupid'],
                    groupName: data.documents[0]['groupname'],
  ...
  getrequests() async {
    ...
    //  print(data.documents[0]['groupid']);
    ...
    if (data.documents.length != 0 && data.documents != null) {
      for (int i = 0; i < data.documents.length; i++) {
        if (tripid[i] == data.documents[i]['tripid']) {
          print("same trip id");
        } else {
          setState(() {
            if (tripid.contains(data.documents[i]['tripid'])) {
              print("already exist");
            } else {
              _showNotification();
              tripid.add(data.documents[i]['tripid']);
            }
          });
        }
      }
