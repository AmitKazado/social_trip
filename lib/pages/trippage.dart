    ...
    await DatabaseService()
      ...
      if (value.docs.length != 0) {
        ...
        for (int i = 0; i < value.docs.length; i++) {
          ...
          setState(() {
            data = value.docs[i]['status'];
            if (data == 'pending') {
              print("im here first");
              statuss = 'pending';
              if (value.docs[i]['index'] - 1 < 0) {
                status.insert(value.docs[i]['index'], '2');
              } else if (i == 3 || i == 4 || i == 5) {
                status.insert(value.docs[i]['index'] - 1, '2');
              } else {
                status.insert(value.docs[i]['index'], '2');
              }
              ...
              if (value.docs[i]['index'] - 1 < 0) {
                ...
                status.insert(value.docs[i]['index'] - 1, '1');
              }
              print("the status are $status");
            } else {
              statuss = 'Join';
              if (value.docs[i]['index'] - 1 < 0) {
                ...
                status.insert(value.docs[i]['index'] - 1, '0');
              }
              print("the status are $status");
            }
          });
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return isloading
                        ...
                        : ListTile(
                            ...
                            title: Text(
                              _destructureName(
                                  snapshot.data.docs[index]['tripname']),
                              style: TextStyle(
                                color: Colors.white,
                              ),
                            ),
                            ...
                            trailing: status[index].isNotEmpty
                                ? InkWell(
                                    onTap: () async {
                                      print(
                                          "deleting ${widget.groupId + '_' + snapshot.data.docs[index].get('tripId')}");
